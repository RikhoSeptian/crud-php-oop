...
if (!empty($_GET['kategori'])) {
  ...
  $row = $config->prepare($sql);
  $row->execute($data);
  echo '<script>window.location="../../Kategori/kategori.php"</script>';
}
...
if (!empty($_GET['gambar_barang'])) {
  ...
  if (!in_array($filetype, array_keys($allowedTypes))) {
    echo '<script>alert("You can only upload JPG, PNG and GIF file");window.location="../../Produk/edit.php"</script>';
    exit;
    ...
    // echo "<font face='Verdana' size='2' ><BR><BR><BR>
    // 		<a href='../../index.php?page=user'>Back to upform</a><BR>";
    echo '<script>alert("WARNING !!! Besar Gambar Tidak Boleh Lebih Dari 4 MB");window.location="../../Produk/edit.php"</script>';
    exit;
  } else {
    $dir = '../../assets/gambar/';
    ...
    if (move_uploaded_file($tmp_name, $dir . $name)) {
      ...
      unlink('../../assets/gambar/' . $foto2 . '');
      //input foto
      $id = $_POST['id'];
      $data[] = $name;
      $data[] = $id;
      $sql = 'UPDATE barang SET gambar_barang=?  WHERE barang.id_barang=?';
      $row = $config->prepare($sql);
      $row->execute($data);
      echo '<script>window.location="../../Produk/produk.php"</script>';
    } else {
      echo '<script>alert("Masukan Gambar !");window.location="../../Produk/produk.php"</script>';
      exit;
    }
  }
}
...
if (!empty($_GET['gambar'])) {
  ...
  if (!in_array($filetype, array_keys($allowedTypes))) {
    ...
    if (move_uploaded_file($tmp_name, $dir . $name)) {
      ...
      echo '<script>alert("Masukan Gambar !");window.location="produk.php"</script>';
      exit;
    }
  }
}
...
if (!empty($_GET['jual'])) {
  ...
  if ($hasil['stok'] > $jumlah) {
    ...
    $row1->execute($data1);
    echo '<script>window.location="../../Penjualan/jual.php"</script>';
  } else {
    echo '<script>alert("Keranjang Melebihi Stok Barang Anda !");
					window.location="../../Penjualan/jual.php"</script>';
  }
}
